    ...
    state = ProjectState(
        ...
        tasks=(
            Task(
                id=new_task_id(),
                objective="Fix",
                status=TaskStatus.AGENT_REPORTED_DONE,
            ),
        ),
    )
...
def test_task_verification_is_not_project_acceptance(independent: bool) -> None:
    proof = Evidence(
        ...
        provenance=Provenance(
            source="test", scope="synthetic fixture", created_at=datetime(2026, 9, 17, tzinfo=UTC)
        ),
    )
    task = Task(
        id=new_task_id(), objective="Fix", status=TaskStatus.PASSED, linked_evidence=(proof.id,)
    )
    state = ProjectState(
        project_id="p",
        tasks=(task,),
        evidence=(proof,),
        verifications=(VerificationRecord(task_id=task.id, passed=True, evidence_id=proof.id),),
    )
    view = build_project_view(state, repository(), project_id="p", captured_at="2026-09-17")
...
def test_projection_rejects_wrong_project_id() -> None:
    with pytest.raises(ValueError, match="project id"):
        build_project_view(
            ProjectState(project_id="other"), repository(), project_id="p", captured_at="2026-09-17"
        )
