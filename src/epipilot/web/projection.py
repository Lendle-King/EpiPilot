    ...
    *,
    ...
    if state is not None:
        ...
        for item in state.evidence:
            ...
            evidence.append(
                Card(
                    ...
                    details={
                        "证据类别": item.kind.value,
                        "来源": item.provenance.source,
        ...
        for task in state.tasks:
            ...
            verified = bool(
                task.status.value == "passed"
                and latest
                and latest.passed
                and proof
                and proof.independently_verified
                and proof.kind.value != "executor_report"
            ...
            card = Card(
                ...
                if task.status.value != "passed" or verified
                else "needs_review",
                summary="历史独立验证通过；合并、部署和当前版本适用性另行判断。"
                if verified
        ...
        for hypothesis in state.hypotheses:
            refs = tuple(
                SourceRef(kind="evidence", location=str(value))
                for value in (*hypothesis.supporting_evidence, *hypothesis.contradicting_evidence)
            )
            card = Card(
        ...
        for fact in state.facts:
            knowledge.append(
                Card(
                    ...
                    details={
                        "有效起点": fact.valid_from.isoformat(),
                        "有效终点": fact.valid_to.isoformat() if fact.valid_to else "未关闭",
                        "来源": fact.provenance.source,
                        "范围": fact.provenance.scope,
                    },
                    refs=tuple(
                        SourceRef(kind="evidence", location=str(value))
            ...
            decisions.append(
                Card(
                    ...
                    details={
                        ...
                        "决策权属": decision.authority.value,
                        "可撤销": "是" if decision.reversible else "否",
                    },
                    refs=tuple(
                        SourceRef(kind="decision_basis", location=value)
    ...
    if not repository.documents:
        opportunities.append(
            Card(
                ...
                refs=(
                    SourceRef(kind="repository_scan", location=".", revision=repository.revision),
                ),
            )
        )
    return ProjectView(
        ...
        event_version=state.event_version if state else 0,
        repository=repository,
        summary="从项目主线、目标差距与证据出发，决定下一步。"
        if goals
        else "已建立静态项目地图；当前目标与执行结果尚需接入正式项目状态。",
        goals=tuple(goals),
